...
def render_cognitivo():
    ...
    col1.metric("Presión Colector", f"{presion:.1f} kPa", delta="SEGURO (VENT)" if (decision and "Cierre" in str(decision) and escenario == "B") else ("CRÍTICO" if presion >= cfg.PRESION_MAX_PLANTA else "Estable"))
    col2.metric("Nivel Separador", f"{nivel_sep:.1f} %", delta="CONTENIDO (ESD)" if (decision and "Cierre" in str(decision) and escenario == "B") else ("DISPARO ESD" if nivel_sep >= cfg.NIVEL_MAX_SEPARADOR else "Normal"))
    col3.metric("Temperatura Reboiler", f"{temp_reboiler:.1f} °C", delta="EN ENFRIAMIENTO" if (decision and "Cierre" in str(decision) and escenario == "C") else ("DEGRADACIÓN TEG" if temp_reboiler >= cfg.TEMP_MAX_REBOILER else "Normal"))
    col4.metric("Calidad Gas", f"{calidad_gas:.1f} mg/m³", delta="FUERA DE NORMA" if calidad_gas > cfg.LIMITE_HUMEDAD else "OK")

    if decision:
        st.markdown("---")
        if "Cierre de Emergencia Manual" in decision:
            st.success("🎯 **Excelente resolución cognitiva.** De acuerdo con los manuales de seguridad operativa y la NAG-125, ante la inminencia de un daño mecánico catastrófico, la prioridad mandatoria es detener la planta en forma segura.")
        else:
